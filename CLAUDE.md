...
src/lib/agents/              the agent runtime (Task 6)
  ...
  provider.ts                the one place a credential becomes a model handle (Task 6c: the Agent SDK branch)
  model.ts                   RunModel — a model and which transport it is on
  mcpTools.ts                a run's tool set as the one in-process MCP server the SDK may call
  stubModel.ts               a scripted model for tests; refused outside development/test
vendor/claude-code-bridge/   the Agent SDK bridge as a workspace package with its own zod 4 (see its index.js)
...
src/worker/handlers/research.ts  the research job: facts, breadth, the run, unreadable unknowns, provenance, ingest, the Event
src/lib/research/            breadth, budget (four caps + the 70% note), corpus, scrub (injection scan), provenance, validate, rubric
src/lib/facts/               the product facts file: schema, loader, hash (Task 11)
...
**The research agent is built to its signed definition in full, and the runtime enforces what the prompt asks for (Task 12).** Four read-only tools through `withReplay` (`agents/research/tools.ts` is the only place a Tavily or Firecrawl client is built); the budget is charged *inside* the replay wrapper so a replayed call is free, and a cap is a typed **unspent** failure that releases its key and ends the run as `cap` naming the field; the 70% re-plan is a `Runtime checkpoint:` line on the next tool result, once per field; the wall-clock cap is a timer on the run's controller. Fetched pages are injection-scanned (`scrub.ts`) before the 12k cap and before storage, and every page and snippet forms the run's corpus, which the provenance check reads before ingest: an item needs an evidence URL in the corpus and a cited number, a six-word quote span, or two of its three most distinctive words on that page, else it is `speculative` with `inferredFrom` saying why; over twenty percent failures re-runs once on the same job with the failures named (every earlier call replays), and a second failure fails the job `bad_output`. Ingest is `validatePack`: `demoteStale`, then the strict schema, then `hook.answeredBy` ⊆ live fact ids. **The loop parses `researchRawSchema` and ingest parses `researchOutputSchema`** — the same rules minus the stale check, because the runtime demotes stale items *after* the model answers; the module minimums are never waived; since v3.2 (definition §10 note 28) the stop rule is reachable because the scope is decided before any floor-bound module is written. A seventh tool, `decideScope`, comes after m01. A `stop` is persisted as its own step and is terminal: nothing more is searched, fetched, written, synthesised or re-asked, even on a retry. The Event then carries `outcome: "insufficient"` with one to three widening options. The rep's structured scope (`brief.scope`) is locked as pack metadata and enforced on m04's seed firms and lead gen recipes (`agents/research/output/scope.ts`). The pack lands on a `research.completed` Event with a `research:<jobId>` SideEffect guard until 6d's table. Model and effort are on the definition (`SPECS`: research Opus 5 high), not in a handler.

**`assertPlainWords` is applied to what an agent wrote, never to what it quoted.** Research v2 §3 says "`assertPlainWords` on every string", and taken literally that is the one use `src/lib/copy/plainWords.ts` documents as wrong: its banned list is ordinary English and thirteen first names, so a buyer quote mentioning a pipeline, a firm called Vector Capital and the Lusha facet names in `recipe` would all be refused for being true. Each output schema therefore exports an `authoredText` function naming the strings the agent itself composed, and checks those. Outreach is the exception and keeps the check on the whole body, against the prose list (`assertPlainProse`), because §12 row 12 is explicit about it and a false positive there costs one redraft, which §7 already budgets for.
...
- Provider credentials: `createAnthropic({ apiKey })` sets `x-api-key`; that is the API-key fallback path. The subscription token never goes to `@ai-sdk/anthropic` at all (see the Agent SDK section below).
...
### Two transports, one ledger — the Claude Agent SDK bridge (`ai-sdk-provider-claude-code@4.3.1` over `@anthropic-ai/claude-agent-sdk@0.3.263`, Task 6c, verified live 2026-09-09)

Read off the installed packages and confirmed on the wire with the subscription token. Re-check on upgrade.

- **Why it exists.** The subscription token (`claude setup-token`) is refused by the Messages API (three 429s, empty message, both models; `reviews/2026-09-09-spike-runtime-live-D2.md`) and accepted by the Agent SDK. `makeModel` routes `CLAUDE_CODE_OAUTH_TOKEN` to the SDK and `ANTHROPIC_API_KEY` to the Messages API; token first when both are set, and the SDK subprocess is handed the token and explicitly **not** the key.
- **The zod gate.** The bridge and the SDK peer on zod 4; Relay pins zod 3.25.76. `vendor/claude-code-bridge` is a workspace package declaring the bridge and zod 4 as its own dependencies, so npm nests both under `vendor/claude-code-bridge/node_modules` and the root stays on zod 3. `npm ci` is clean with no flags. The bridge accepts a **zod 3** object schema as a tool's `inputSchema` (it reads `.shape`), so Relay's tools are unchanged.
- **The loop moves inside the SDK.** `generateText` sees **one** call for the whole run, with usage summed over every turn, and `stopWhen` never fires; the SDK's `maxTurns` is the cap (the definition's `maxModelSteps`), and reaching it ends the query with result subtype `error_max_turns` before the bridge throws a generic `APICallError`. Tools are not executed by the AI SDK on this path — the bridge ignores AI SDK `tools` — so `runAgent` does not pass them; they reach the model as an **in-process MCP server** (`createAiSdkMcpServer`, wrapped by `src/lib/agents/mcpTools.ts`) whose handlers are the tools' own `execute`, in this process, through `withReplay`. Tool steps and replay keys are therefore identical on both transports.
- **The ledger reads the SDK's message stream.** `onSdkMessage` relays every assistant message with the Messages API's own `usage` block, per message, and a closing `result` with `modelUsage` (per-model totals, including calls the stream does not show) and `total_cost_usd`. `runAgent` records one `model` step per assistant message from the wire block, priced from the pinned table, and skips the AI SDK's summed call. Then it reconciles: whatever a model's `modelUsage` exceeds the recorded turns by becomes one more `model` step under that model's canonical id. Observed: the SDK makes a **Haiku** call of its own on every run (~900 in, ~15 out), and a structured-output turn can land in `modelUsage` without a relayed message; both end up as rows, and the run total equals the SDK's `total_cost_usd` to the micro-dollar (`scripts/spike/cost-check.ts` checks this per model). A model the price table does not know fails the run rather than record zero.
- **Cache writes are one-hour.** Every turn on this transport carries `cache_creation.ephemeral_1h_input_tokens`, billed at **2x** input (the Messages API default is five-minute at 1.25x). `pricing.ts` carries both rates and `StepUsage.tokensCacheWrite1h` says how many of a step's writes were one-hour. Pricing them all at 1.25x under-charged by 37.5% of cache writes.
- **Isolation, pinned by `tests/agents/provider.test.ts`.** `settingSources: []`, `env.CLAUDE_CONFIG_DIR = RELAY_AGENT_HOME` (default `~/.relay/agent-home`, created on first use), `tools: []` (no built-in tool at all), `mcpServers` = the one Relay server, `allowedTools` = exactly the run's tools as `mcp__relay__<name>`, `permissionMode: "bypassPermissions"` with the SDK's acknowledgement flag, `persistSession: false`. Verified live: asked to read `/etc/hostname` with no tools declared, the model answered `NO_FILE_ACCESS` and the SDK's tool list held nothing but the MCP names. The SDK spawns its own bundled binary; nothing needs `claude` on `PATH` (verified with `PATH=/usr/bin:/bin`).
- **Model ids.** The bridge knows the aliases `opus`, `sonnet`, `haiku`, `fable` and warns "Unknown model ID" for a full id such as `claude-opus-5`, then proceeds with it; the SDK's init message confirms the full id. The warning is noise, not a fallback. `modelUsage` keys can be dated aliases (`claude-haiku-4-5-20251001`); `canonicalModelId` strips the date for the price table.
- **Bundling.** `tsup` leaves `@relay/claude-code-bridge` external: the SDK finds its binary relative to its own package, so `dist/worker/main.js` needs `node_modules/` and `vendor/` beside it, as it already needs `agents/`.
