...
## 13. Amendment A1: research v3.2 integration (approved by the product owner, 2026-09-12)
Research as signed in research v3 with notes 28 and 29 replaces this definition wherever the two differ.
1. **Input.** A research job's input is `{ brief, priorRun?, priorPackIds? }` and nothing else. `brief` is research's own brief: `product`, `motion`, `who` (the rep's words, verbatim), `region` (ISO-3166 alpha-2), `howMany`, `weeks`, `channels[]`, optional `scope`, optional `existingCustomers`. `scope` holds only fields the rep supplied or confirmed: `countries`, `places` (with aliases), `orgTypes`, `size { unit, min?, max? }`, `roles { include?, exclude? }`. There is no breadth, and the facts version is pinned by the research runtime, not supplied. The job row carries `campaignId` and `briefVersion` as columns.
2. **Result.** Research writes one Event, `research.completed`, per job; `after.outcome` is `complete`, `partial` or `insufficient`. There is no `research.stopped` Event and no `valid | invalid_output` result. `complete` → Plan ready. `partial` (a rail ended the run) → Plan ready with the missing modules shown; the rep may confirm or edit the brief. `insufficient` → Researching (stopped): a successful result, not a fault; the job is `done`, and `not_enough_evidence` is not a needs-you reason for it. A research job that ends `failed` → needs you (`took_too_long` or `bad_output`, from the job's error); nothing is spent.
3. **Timing.** §4's 20-minute research timeout is removed. Research runs under its own rails (90-minute wall clock, then a partial pack; research §6). The queue lease is extended while it runs, and a retried job resumes. Expected duration is 20 to 45 minutes; any backstop timeout is 100 minutes, above the rail.
4. **Widen** (from Researching (stopped)): the rep chooses one of the one to three options research returned (region, size, sector or role; never pain, never padded to three); its `scopePatch` is applied to `brief.scope` (`null` removes a constraint); `briefVersion + 1`; research is enqueued with `priorPackIds: [the stopped result's research.completed Event id]` and `priorRun: { insufficient: true, widenedBy: <the option's dimension>, note: <the option's text> }`.
5. **Edit brief** replaces "Change something" (from Plan ready, stopped, or needs you): the rep edits the brief's own fields and scope; `briefVersion + 1`; research is enqueued with `priorPackIds: [the campaign's latest research.completed Event id]` when there is one, and no `priorRun`. There is no reason picker, and no model reads anything.
6. **Try again** (needs you): the existing failed research job is requeued for the same `briefVersion`, with the same input and idempotency key. The queue's existing attempt accounting is reused; there is no second retry or attempt model. If the failed job has exhausted its permitted attempts, the rep's explicit action permits one additional attempt. `campaign.research_retried` is recorded.
7. **Idempotency.** A campaign's research job is keyed on its campaign and brief version (`campaign:<campaignId>:research:v<briefVersion>`); the version is also a column on the job and is never read back out of the key.
8. **Campaign state is derived** from the campaign, its latest research job at its brief version, and that job's `research.completed` Event. It is not stored while research is the only asynchronous step.

## 14. Amendment A2: lead gen v2.1 integration (approved by the product owner, 2026-09-14)
Lead gen v2.1 (`leadgen.v2.1.signed.md`) replaces this definition wherever the two differ.
1. **Flow.** Plan ready → Confirm plan → Finding people → People found → Reveal emails → People ready. This replaces §5's rows "Plan ready | Confirm plan" and "Finding people | Reveal". Reveal enqueues no `draft`: People ready is terminal for this slice, and Drafting, Running and Done are not reachable yet.
2. **Confirm plan** is the first spend gate and authorises capped search spend only. The server reads the provider balance, then one `mutate` writes `campaign.confirmed` carrying `LeadGenHandoffV1` (lead gen v2.1 §3) and enqueues `lead_gen`, keyed `campaign:<campaignId>:lead_gen:v<briefVersion>`. The buyer group in the handoff is chosen at the campaign boundary (H1: Research's top-ranked campaign candidate); lead gen only reads it.
3. **Lawful basis.** "record LIA" in §5 is replaced by the lawful-basis record in the handoff: the exact text the rep confirmed, the actor, the time and the brief version (lead gen v2.1 §12). It is shown as "Lawful basis confirmed" and is not described as an LIA. Live production processing waits for the open lawful-basis decision (master PART IX).
4. **Reveal emails** is the second spend gate: `campaign.reveal_confirmed`, then `reveal` jobs keyed `campaign:<campaignId>:reveal:v<briefVersion>:b<batch>`.
5. **State stays derived** (extends A1 item 8) from the campaign, its brief version and, at that version, the confirm Event, the latest `lead_gen` and `reveal` jobs and their Events (lead gen v2.1 §11): findingPeople, peopleFound (including X of N), revealing, peopleReady, needs you. Chips: Finding people, People found, People ready, Needs you.
6. **Needs-you reasons** gain lead gen v2.1 §11's halt reasons; §7's `credits_capped` is its `over_cap`.
