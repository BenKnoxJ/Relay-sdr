...
| 3 | Opener honesty | `usable: false` → role-problem opener and "no person fact" on the card; no proper noun in the opener absent from lookup or pack |
| 4 | Ask and length | 100% of drafts have exactly one question, last; **Email 1 is 40 to 110 words (v2.1)**; follow-ups shorter than the last; LinkedIn and call shapes within limits |
...
| 9 | Redraft (v2.1) | a Tier A rejection redrafts once, with findings; a second failure goes to Needs you with the labels. At most two model generations per draft |
| 10 | Lookup bounds | never more than 2 + 2, a maximum and not a quota; replay makes zero new calls; injection fixture stripped and logged |
