...
**v3 · SIGNED by Benny-san 2026-09-09 (in session: "okay lets continue with the plan then with those changes. build it!") · supersedes v2 (signed 2026-09-08) · basis: `reviews/2026-09-09-audit-research-vs-signal.md`, `plans/2026-09-09-research-v3-plan.md`, independent plan review of 2026-09-09**
...
| 12 | Depth against the bar | brief A read module by module against Signal's May insurance pack by Benny-san; brief E against the legal exemplar; nothing the campaign agent needs is missing |
...
1. Facts-only claims allowed as `strong` with source `facts://i360.<id>` and `primary: true`. 2. ISO codes internally, country names on screen. 3. Partial dates accepted (2026-09-09). 4. `existingCustomers` is the eighth and last campaign-start question (2026-09-09). 5. Sub-agents out of v3 pending a ledger test (2026-09-09). 6. **Depth fixture, a fourth exemption** (§7; Benny-san, 2026-09-10): fields v3 added that the May pack predates — m02 `doNothing`, m09 angle `confidence` and `channelFit`, m18 `queriesTried` on a `not-found` unknown, m19 `factsVersion`. They stay v3 rules for every run; the fixture is exempt from them by name in its manifest. 7. **`onlyModules`** (§2; Benny-san, 2026-09-10): a runtime input beside `provenanceRerun`, set only by the bench (`--modules`), so a run writes m00 and the named modules and nothing else; a job never sets it. 8. Dated-event dates (m13) take the §3 date formats (2026-09-10). 9. **m11 "not today" answers** (Benny-san, 2026-09-10): an objection may cite planned facts in `notYetFactIds` as the reason something is not available, never as a promise; `factIds` stay live-only. 10. **Provenance skips an honest guess** (2026-09-10): a `speculative` claim with no url and an `inferredFrom` has nothing to check against a page and is not counted in a module's failure share. 11. **No ceilings on field text** (2026-09-10): structured text fields carry rails an order above a real sentence (prose 8,000, labels 500 characters), per §3's "floors, no ceilings".

**v3.1 — from the brief E audit (`reviews/2026-09-10-audit-research-v3-brief-e.md`; Benny-san 2026-09-10: "might as well do all").** 12. **Cross-module references are checked on write**: a module naming m03 archetypes, m05 pains, m04 seed firms or echoing m00's filters is checked against the modules already accepted, not only at ingest. 13. **m06 `voice`** (`practitioner` | `firm-document` | `representative-body` | `regulator` | `adviser` | `vendor`) on every phrase; `notBuyer` is false only for a practitioner; the ≥60% buyer-words share counts practitioners with a role. 14. **m04 seeds**: at least one seed per kind of buyer sized `confirmed` or `estimated` with a source url; no more than half of all seeds from one list page. 15. **m18**: `askOnFirstCall` required on `not-found` and `conflicting` unknowns. 16. **A re-ask edits, not rewrites**: `provenanceRerun.current` carries the stored version of each re-asked module. 17. **Two phases per job**: `research` (the definition's model; all tools) writes m00, m01, m02, m03, m05, m06, m04, m10, m12, m13, m17; `synthesis` (Sonnet 5, a fresh context holding only the accepted modules, search and fetch closed) writes m07, m08, m09, m11, m14, m15, m16, m18, execSummary, repSummary. Inputs `phase` and `acceptedModules` are runtime fields. 18. **m19 is assembled by the runtime** from every url the pack cites; the model does not write it. 19. **Provenance checks every url field** (m01 bodies, m02 competitor urls, m04 list sources and size sources, m10 entries, m12 sources, m13 sources) against the pages and snippets read, and a claim carrying figures passes only if **every** non-year figure is on a page it cites. 20. **Never-say list** per product (`facts/<product>.v<n>.never-say.json`, from the facts notes) linted on write and at ingest over what the agent authored. 21. Fetched page text capped at 8,000 characters (was 12,000). 22. **Sign-off set** (§8; Benny-san 2026-09-10, to cut cost and time): brief A in full (v3.1 proven live and read against Signal's May insurance pack, row 12); brief C in full (the insufficient stop, row 8); brief D as `--modules m00,m04,m14` with A as the prior pack (widening, no repeated firms, what changed, row 10); brief B replaced by a relevant channel market — UK telephony resellers and 3CX partners — as `--modules m00,m03,m09` (channel framing and partner economics); brief E's first run plus the offline replay of its modules stands in for its re-run. "Zero insufficient modules" applies to A in full and to the modules each partial run writes. 23. **Refuse for content, correct the mechanics** (Benny-san 2026-09-10: "the main thing is the time to deliver plus all the failures"): on write the runtime corrects and reports what it can derive — a confidence word above its evidence's ceiling is lowered to it, `domains` set to the hosts of the urls, country names mapped to ISO codes, a missing `claims` list set empty, a market-size line written as an object joined into one line — and refuses only for content (floors, cross-module ids, never-say, fact ids, seed sizing). A refusal may be answered with `fixes` (`{ path, value }` pairs) applied to the last version sent, instead of the whole module. Item `speaker` and `role` rails 500 characters. The never-say lint also reads negation after the phrase ("…as not shipped"). 24. **Row 12 signed (Benny-san 2026-09-11; `reviews/2026-09-11-signoff-research-row12.md`)**: brief A v3.1 PASS — meets the May insurance bar and exceeds it in provenance discipline, contradiction handling, product-boundary control and machine-usable structure; brief A becomes the **regression fixture** and the May pack stays the **external depth floor**; for a new sector the human exemplar stays the bar until a Relay pack for that sector is signed. Brief E first run PASS on content depth only — not a v3.1 rerun sign-off. 25. **m07 `strength: direct | partial`** on every mapping (a partial mapping says what it does not change in `mustNotImply`); the invariant is "every pain accounted for — direct, partial or unmatched", not "every pain maps to something shipped". 26. **Stable ids** on m09 angles, m10 venues, m11 objections, m12 contact rules and m13 dated events (the runtime assigns one when missing), and **m16 candidates reference them** — `painIds`, `angleIds`, `objectionIds`, `eventIds`, `venueIds`, `contactRuleIds` — checked on write against the accepted modules and the candidate's own kind of buyer. 27. **Facts v2 signed 2026-09-11**: `i360.proof.tenant-isolation-independently-audited` renamed `…-internally-audited`; the note naming a former trial customer no longer names it. v1 stays as signed.

---
**Signature block.** Signed by Benny-san 2026-09-09, with the two flagged recommendations taken: the primary-source carve-out on the per-module domain cap, and the top-ranked campaign candidate as the slice-1 default archetype, changeable on the card. On signature this file is renamed `research.v3.signed.md`, copied verbatim to `agents/research/definition.md`, and the master doc amendment in `plans/2026-09-09-master-doc-amendment-research-v3.md` is applied.
