...
export function PeopleTab({
  ...
  const filtered = filter.status !== null || filter.due || filter.q !== "";
      ...
      <Card label={c.listLabel} aside={<span className="type-small text-muted" data-testid="people-count">{`${shown.length} ${c.of} ${rows.length} ${c.shown}`}</span>}>
        ...
        {shown.length === 0 ? (
          <div data-testid="people-empty">
            ...
            {filtered ? (
              <p className="mt-2 text-center">
                <Link href={peopleHref(campaignId, { status: null, due: false, q: "" })} scroll={false} onClick={() => setQ("")} className="type-small text-action underline">
                  {c.clearFilters}
                </Link>
              </p>
            ) : null}
          ...
          <ul data-testid="people-list" className="-mx-card divide-y divide-line border-t border-line">
            {shown.map((row) => (
              <PersonRow key={row.campaignPersonId} row={row} today={today} href={peopleHref(campaignId, filter, row.campaignPersonId)} open={person?.campaignPersonId === row.campaignPersonId} />
            ))}
          </ul>
        )}
      </Card>
...
function PersonRow({ row, today, href, open }: { row: TrackingRow; today: IsoDate; href: string; open: boolean }) {
