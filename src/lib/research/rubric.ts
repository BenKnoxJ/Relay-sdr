 ...
 * Benny-san's, read module by module against Signal's May pack. Each row
...
export function scoreRubric(input: RubricInput): RubricRow[] {
  ...
  if (last === undefined) {
    rows.push({ check: 3, name: "Provenance", verdict: "n/a", detail: "no provenance report supplied (the five hand spot-checks are Benny-san's)" });
  } else {
  ...
  // 12 — depth against the bar: Benny-san's.
  rows.push({ check: 12, name: "Depth against the bar", verdict: "n/a", detail: "Benny-san reads brief A against Signal's May insurance pack and brief E against the legal exemplar" });
 ...
 * accurately is Benny-san's read; this row checks it records them.
 */
