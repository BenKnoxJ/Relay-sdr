...
describe("the price table", () => {
  it("is the table that was reviewed", () => {
    ...
    // MTok, Sonnet 5 at $2/$10, cache reads at 0.1x input and cache writes at
    // 1.25x for the five-minute TTL.
    expect(PRICE_TABLE_SHA256).toBe("32912205516964a6210a49d3d2d81c3fbee9a309a92ee0be46b615523e790004");
  });

  it("prices the two models §24 pins, and refuses anything else", () => {
    expect(Object.keys(PRICES).sort()).toEqual(["claude-opus-5", "claude-sonnet-5"]);
...
describe("cost", () => {
  ...
  it("does not price the input total, which already contains the cache figures", () => {
    ...
    const withTotal = cost(
      { tokensIn: 999_999, tokensInUncached: 1_000, tokensCacheRead: 0, tokensCacheWrite: 0, tokensOut: 0 },
      "claude-opus-5",
    );
