...
export function goodPack(options: { liveFactId: string }): PackShape {
  ...
  const modules = {
    ...
    m07: {
      ...
      claims: [],
      mappings: ARCHETYPES.flatMap((a) => pains[a].map((p) => ({ painId: p.id, capability: "Every call scored", strength: "direct", factIds: [live] }))),
      unmatched: [],
    },
    m08: {
      status: "complete",
    ...
    m09: {
      ...
      perArchetype: ARCHETYPES.map((a) => ({
        archetypeId: a,
        angles: [1, 2, 3, 4, 5].map((rank) => ({ id: `${a}-angle-${rank}`, rank, text: `angle ${rank}`, confidence: "weak", channelFit: ["email"] })),
    ...
    m10: {
      ...
      claims: [],
      entries: [1, 2, 3, 4, 5].map((i) => ({ id: `venue-paper-${i}`, kind: "press", name: `Insurance paper ${i}`, url: `https://press-${i}.example/`, audience: "Claims leaders", why: "They read it." })),
    },
    m11: {
      status: "complete",
      body: "Objections.",
      claims: [],
      perArchetype: ARCHETYPES.map((a) => ({ archetypeId: a, objections: [1, 2, 3].map((i) => ({ id: `${a}-objection-${i}`, objection: `objection ${i}`, factIds: [] })) })),
    },
    m12: { status: "complete", body: "Contact rules.", claims: [], rules: [{ id: "rule-email-1", channel: "email", region: "GB", rule: "PECR: corporate subscribers may be emailed.", source: "https://ico.org.uk/", bars: false }] },
    m13: {
      status: "complete",
      body: "Dated events.",
      claims: [],
      entries: [1, 2, 3].map((i) => ({ id: `event-${i}`, date: RECENT, what: `event ${i}`, source: `https://events-${i}.example/`, why: "Inside the campaign." })),
      noneFound: false,
      queriesTried: [],
    },
    ...
    m16: {
      ...
      candidates: ARCHETYPES.map((a, i) => ({
        ...
        painIds: [`${a}-pain-1`],
        angleIds: [`${a}-angle-1`],
        objectionIds: [`${a}-objection-1`],
        eventIds: ["event-1"],
        venueIds: ["venue-paper-1"],
        contactRuleIds: ["rule-email-1"],
      })),
    },
