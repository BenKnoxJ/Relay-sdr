...
describe("the actions the page offers are the ones the server accepts", () => {
  ...
  it("@proof retries a reveal that failed before any request left Relay, once, and reveals", async () => {
    ...
    expect(stopped.activity?.some((entry) => entry.line.startsWith(campaignsCopy.activityRevealConfirmed))).toBe(true);
...
describe("the campaign summary", () => {
  it("@proof counts people, emails, and search, reveal and research spend apart, the same on the list as on the page", async () => {
    ...
    const lines = page.activity?.map((entry) => entry.line) ?? [];
    expect(lines[0]).toContain(campaignsCopy.activityRevealed);
    expect(lines.some((line) => line.startsWith(campaignsCopy.activityRevealConfirmed))).toBe(true);
    expect(lines.some((line) => line.startsWith(campaignsCopy.activityConfirmed))).toBe(true);
    expect(lines).toContain(campaignsCopy.activityCreated);
  });
