...
describe("campaigns.get and campaigns.list", () => {
  it("draws a new campaign as Researching, with nothing downstream pretending to have happened", async () => {
    ...
    expect(campaign.activity?.map((entry) => entry.line)).toEqual([campaignsCopy.activityCreated]);
  });
  ...
  it("is Plan ready when research completes, with the plan cards from the real pack", async () => {
    ...
    // The pack stays on the server: the page reads the plays and the Overview (final MVP pass).
    expect(campaign.pack).toBeNull();
    expect(campaign.overview?.groups.length).toBe(3);
    expect(campaign.facts?.research).toMatchObject({ outcome: "complete" });
